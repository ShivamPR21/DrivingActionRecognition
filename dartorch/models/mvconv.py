...
class MultiViewConvBackbone(nn.Module):
    ...
    def __init__(self, n_views: int = 3, n_channels: int = 1, n_frames: int = 5, in_size: Tuple[int, int] = (128, 128)) -> None:
        super().__init__()
        ...
        self.conv1 = Conv2DNormActivation(self.n_views*self.n_channels,
                                          64,
                                          kernel_size=5,
                                          stride=2,
        ...
        self.res2 = Conv2DResidualBlock(64, 64,
                                         kernel_size=3,
                                         stride=1,
                                         activation_layer=nn.ReLU6)

        self.res3 = Conv2DResidualBlock(64, 64,
                                         kernel_size=3,
                                         stride=1,
                                         norm_layer=nn.BatchNorm2d,
                                         activation_layer=nn.ReLU6)
        ...
        self.res4 = Conv2DResidualBlock(64, 128,
                                         kernel_size=3,
                                         stride=2,
                                         activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1

        self.res5 = Conv2DResidualBlock(128, 128,
                                         kernel_size=3,
                                         stride=1,
                                         activation_layer=nn.ReLU6)

        self.res6 = Conv2DResidualBlock(128, 128,
                                         kernel_size=3,
                                         stride=1,
                                         norm_layer=nn.BatchNorm2d,
                                         activation_layer=nn.ReLU6)
        ...
        self.res7 = Conv2DResidualBlock(128, 256,
                                          kernel_size=3,
                                          stride=2,
                                          activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1

        self.res8 = Conv2DResidualBlock(256, 256,
                                         kernel_size=3,
                                         stride=1,
                                         activation_layer=nn.ReLU6)

        self.res9 = Conv2DResidualBlock(256, 256,
                                         kernel_size=3,
                                         stride=1,
                                         norm_layer=nn.BatchNorm2d,
                                         activation_layer=nn.ReLU6)
        ...
        self.res10 = Conv2DResidualBlock(256, 512,
                                          kernel_size=3,
                                          stride=2,
                                          activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1

        self.res11 = Conv2DResidualBlock(512, 512,
                                         kernel_size=3,
                                         stride=1,
                                         activation_layer=nn.ReLU6)

        self.res12 = Conv2DResidualBlock(512, 512,
                                         kernel_size=3,
                                         stride=1,
                                         norm_layer=nn.BatchNorm2d,
                                         activation_layer=nn.ReLU6)
    ...
    def forward(self, x:torch.Tensor) -> torch.Tensor:
        ...
        x = x.split(split_size=self.n_views*self.n_channels, dim=1) # Split the data in sequences ([B, n_views*n_channels, H, W])*n_seq
        x = torch.cat(x, dim=0) # Concatenate the sequences to batch dimension [B*n_seq, n_views*n_channels, H, W]
...
class MultiFrameFeatureMixer(nn.Module):
    ...
    def __init__(self, n_frames: int = 5, n_features: int = 512) -> None:
        ...
        self.conv1 = ConvNormActivation1d(self.n_frames, 16,
                                          kernel_size=5,
                                          stride=1,
                                          padding='stride_effective',
                                          bias=True,
        ...
        self.conv2 = ConvNormActivation1d(16, 64,
                                          kernel_size=3,
                                          stride=1,
                                          padding='stride_effective',
                                          bias=False,
        ...
        self.res3 = ConvResidualBlock1d(64, 128,
                                        kernel_size=3,
                                        stride=2,
                                        activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1

        self.res4 = ConvResidualBlock1d(128, 128,
                                        kernel_size=3,
                                        stride=2,
                                        norm_layer=nn.BatchNorm1d,
                                        activation_layer=nn.ReLU6)
        ...
        self.res5 = ConvResidualBlock1d(128, 128,
                                        kernel_size=3,
                                        stride=2,
                                        activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1

        self.res6 = ConvResidualBlock1d(128, 128,
                                        kernel_size=3,
                                        stride=2,
                                        norm_layer=nn.BatchNorm1d,
                                        activation_layer=nn.ReLU6)
        ...
        self.res7 = ConvResidualBlock1d(128, 128,
                                        kernel_size=3,
                                        stride=2,
                                        activation_layer=nn.ReLU6)
        self.out_size = (self.out_size-1)//2+1
