...
def _get_multi_view_frames(
    readers: List[imageio.plugins.ffmpeg.FfmpegFormat.Reader],
    frame_no: int,
    n_frames: int,
...
class DARDatasetOnVideos(Dataset):
    def __init__(
        ...
        self.remove_unlabeled = (
            remove_unlabeled  # Whether to remvoe unlabeled data(-1 class_id)
        )
        ...
        ] = None  # user_id/session_id : view : (video_path, labels)
        ...
        for user in self.user_ids:
            ...
            for session_id_ in session_ids:
                label_df = pd.read_csv(
                    os.path.join(os.path.join(id_dir, session_id_), "labels.csv"),
                    index_col=0,
                )
                videos_df = pd.read_csv(
                    os.path.join(os.path.join(id_dir, session_id_), "vid_files.csv"),
                    index_col=1,
                )
                ...
                for view in self.views:
                    view_file_paths[view] = os.path.join(
                        id_dir, videos_df.loc[view].values[0]
                    )
                    if self.load_reader_instances or self.mode == "test":
                        view_file_paths[view] = imageio.get_reader(
                            view_file_paths[view], "ffmpeg"
                        )
                ...
                n_frames = int(
                    view_file_paths[view].get_meta_data()["fps"]
                    * view_file_paths[view].get_meta_data()["duration"]
                )

                self.labels[session_key] = (
                    view_file_paths,
                    np.array(label_df.values, dtype=np.int32),
                )
                self._n_labels[session_key] = (
                    label_df.shape[0] if self.mode == "train" else n_frames
                )
    ...
    def __get_data(
        self, readers: List[imageio.plugins.ffmpeg.FfmpegFormat.Reader], frame_no: int
    ) -> torch.Tensor:
    ...
    def __getitem_train__(self, index: int) -> Any:
        session_key, idx = self.reduce_idx(
            index
        )  # Find out relevant data directory, dataframe, and reduced index

        reader_dict, label = self.labels[session_key]
        readers = [
            val if self.load_reader_instances else imageio.get_reader(val, "ffmpeg")
            for _, val in reader_dict.items()
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--root_dir",
        default=os.path.join(__HOME_DIR__, "AI-City/A1"),
        help="Root directory path, for AI-City track3 dataset with folder A1, A2, and B",
    )
    parser.add_argument(
        "--load_reader_instances",
        action="store_true",
        help="If given, then video reader instances will be created and stored in advance.",
    )
    args = parser.parse_args()

    dataset = DARDatasetOnVideos(
        args.root_dir, load_reader_instances=args.load_reader_instances
    )
